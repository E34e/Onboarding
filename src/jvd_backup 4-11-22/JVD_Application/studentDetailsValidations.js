...
export const forrmikInitialValues = {
  ...
  export const formikValidations = Yup.object().shape({
    ...
    studentName:Yup.string().required( " Please enter student name").matches(alphaNumeric," Enter valid Service name"),
    motherAadharNo:Yup.string()
    .required("Please Enter Mother Aadhar Number")
    .min(12, " Aadhar number must be exactly 12 digits only")
    .max(12)
    .matches(isValid, "Please Enter  Aadhar Number Correctly"),
    motherToungue:Yup.string().required( " Please enter mother name").matches(alphaNumeric," Enter valid Service name"),
    fatherAadharNo:Yup.string()
    .required("Please Enter Father Aadhar Number")
    .min(12, " Aadhar number must be exactly 12 digits only")
    .max(12)
    .matches(isValid, "Please Enter  Aadhar Number Correctly"),
    fatherName:Yup.string().required( " Please enter father name").matches(alphaNumeric," Enter valid Service name"),
    ...
    parentOccupation:Yup.string().required( " Please enter occupation"),
    ...
    isVhPercent:Yup.string().when('isVh',
    { is: (isVh) => isVh === true ? true : false,
        then: Yup.string().required('select VH percentage'),
        otherwise: Yup.string()
    }),
    isHhPercent:Yup.string().when('isHh',
    { is: (isHh) => isHh === true ? true : false,
      then: Yup.string().required('select hh percentage'),
      otherwise: Yup.string()
    }),
    isOhPercent:Yup.string().when('isOh',
    {
      is: (isOh) => isOh === true ? true : false,
      then: Yup.string().required('select oh percentage'),
      otherwise: Yup.string()
    }),
